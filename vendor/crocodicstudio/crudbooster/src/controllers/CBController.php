...
class CBController extends Controller {
	...
	public function getIndex() {
		...
		$data['totaldata'] = DB::select(DB::raw("SELECT FOUND_ROWS() as total"))[0]->total;
			...
			case 'xls':
				Excel::create($filename, function($excel) use ($response) {
				    ...
				    $excel->sheet($filename, function($sheet) use ($response) {
				    	...
				    	$alphabet = range('A', 'Z');
				    	foreach($response['columns'] as $i=>$c) {
				    		if(isset($c['excel_format'])) {
				    			$sheet->setColumnFormat([
				    				$alphabet[$i]=>$c['excel_format']
				    			]);
				    		}
				    		if(isset($c['excel_width'])) {
				    			$sheet->setWidth($alphabet[$i], $c['excel_width']);
				    		}
				    	}
